...
var cart = {
	...
	init: () => {
		...
		fetch('https://cdn.jsdelivr.net/gh/micakucing/clopzkitchen@main/datainit.json').then(response => {
			if (!response.ok) {
			...
			document.getElementById("title-produk").innerHTML = "Semua Produk kami";
			cart.hPdt.innerHTML = "";
			cart.ps.innerHTML = "";
	...
	list: () => {
		...
		if (empty) {
			...
			item = document.getElementById("template-cart-checkout").content.cloneNode(!0);
			cart.hItems.appendChild(item)
		}
	},
	add: id => {
